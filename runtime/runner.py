...
class WorkflowRunner:
    ...
    def run(self, task_description: str, task_id: str | None = None) -> TaskRecord:
        ...
        try:
            ...
            while retries <= self._config.max_retries:
                dev_report = self._run_developer(record, current_packet)
                if dev_report is None:
                    return self._refresh(record)

                tester_report = self._run_tester(record, dev_report)
                ...
                if tester_decision.action == ACTION_FORWARD and tester_decision.target == TARGET_REVIEWER:
                    reviewer_report = self._run_reviewer(record, dev_report, tester_report)
    ...
    def _run_architect(self, record: TaskRecord, description: str) -> Packet | None:
        ...
        packet = self._invoke(ROLE_ARCHITECT, context, record, "architect")
    ...
    def _run_developer(self, record: TaskRecord, task_packet: Packet) -> Packet | None:
        self._ledger.update_phase(record.task_id, PHASE_DEVELOPER)
        context = task_packet.raw
        return self._invoke(ROLE_DEVELOPER, context, record, "developer")

    def _run_tester(self, record: TaskRecord, dev_report: Packet) -> Packet | None:
        self._ledger.update_phase(record.task_id, PHASE_TESTER)
        context = dev_report.raw
        return self._invoke(ROLE_TESTER, context, record, "tester")

    def _run_reviewer(self, record: TaskRecord, dev_report: Packet, tester_report: Packet) -> Packet | None:
        self._ledger.update_phase(record.task_id, PHASE_REVIEWER)
        context = f"{dev_report.raw}\n\n---\n\nTester validation result:\n\n{tester_report.raw}"
        return self._invoke(ROLE_REVIEWER, context, record, "reviewer")
    ...
    def _run_reshape(self, record: TaskRecord, decision: RouteDecision, original: Packet) -> Packet | None:
        ...
        return self._invoke(ROLE_ARCHITECT, context, record, "architect_reshape")

    def _invoke(self, role: str, context: str, record: TaskRecord, label: str) -> Packet:
        try:
            packet = invoke_agent(self._agent_fn, role, context)
